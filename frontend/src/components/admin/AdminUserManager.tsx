...
export default function AdminUserManager() {
  ...
  if (user?.user_group !== 'admin') return <div className="card-simple bg-slate-50 dark:bg-slate-900/20 border-dashed text-center py-12"><p className="text-slate-500 font-medium">Access restricted to system administrators.</p></div>;
    ...
    <div className="space-y-8">
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          ...
          { label: 'Active Status', value: users.filter(u => u.is_active).length, icon: CheckCircle2 },
          { label: 'Staff/Admins', value: users.filter(u => ['admin', 'instructor', 'ta'].includes(u.user_group)).length, icon: Shield },
        ].map((stat) => (
        ...
        <div className="xl:col-span-2 space-y-6">
          <section className="card-simple">
            <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
              <h2 className="text-sm font-bold uppercase tracking-widest text-slate-400">User Directory</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-6">
              <div className="relative md:col-span-1">
                ...
                <input value={search} onChange={(e) => setSearch(e.target.value)} className="w-full text-xs rounded border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 pl-9 pr-3 py-2.5 outline-none focus:ring-1 focus:ring-accent" placeholder="Search accounts..." />
              </div>
              <select value={roleFilter} onChange={(e) => setRoleFilter(e.target.value as any)} className="text-xs rounded border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 px-3 py-2.5 outline-none">
                <option value="all">All Roles</option>
                {ROLE_OPTIONS.map(r => <option key={r} value={r}>{r.toUpperCase()}</option>)}
              </select>
              <select value={activeFilter} onChange={(e) => setActiveFilter(e.target.value as any)} className="text-xs rounded border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 px-3 py-2.5 outline-none">
            ...
            <div className="overflow-x-auto rounded border border-slate-100 dark:border-slate-800">
              <table className="min-w-full divide-y divide-slate-100 dark:divide-slate-800">
                <thead className="bg-slate-50 dark:bg-slate-900/50">
                  <tr>
                    <th className="px-4 py-3 text-left text-[10px] font-bold uppercase tracking-widest text-slate-400">User Identity</th>
                    <th className="px-4 py-3 text-left text-[10px] font-bold uppercase tracking-widest text-slate-400">Role</th>
                ...
                <tbody className="divide-y divide-slate-50 dark:divide-slate-900 bg-white dark:bg-slate-950/20">
                  {isLoading ? (
                    <tr><td colSpan={3} className="px-4 py-10 text-center text-xs text-slate-400 animate-pulse">Fetching users...</td></tr>
                  ) : users.length === 0 ? (
                    <tr><td colSpan={3} className="px-4 py-10 text-center text-xs text-slate-400 italic">No users found.</td></tr>
                  ) : (
                    users.map((u) => (
                      <tr key={u.id} className="group hover:bg-slate-50/50 dark:hover:bg-slate-900/30 transition-colors">
                        <td className="px-4 py-4">
                          ...
                          <div className="text-[10px] text-slate-400 font-mono mt-0.5">{u.id} · SID {u.sid}</div>
                          <div className="text-[10px] text-slate-500 mt-1">{u.email}</div>
                        </td>
                        <td className="px-4 py-4">
                          <div className="flex items-center gap-2">
                            <select value={roleDrafts[u.id] ?? u.user_group} onChange={(e) => setRoleDrafts({...roleDrafts, [u.id]: e.target.value as UserRole})} className="text-[11px] rounded border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 px-2 py-1 outline-none focus:ring-1 focus:ring-accent">
                              {ROLE_OPTIONS.map(r => <option key={r} value={r}>{r}</option>)}
                            </select>
                            <button onClick={() => void handleRoleSave(u)} disabled={busyUserId === u.id} className="p-1.5 rounded hover:bg-white dark:hover:bg-slate-800 text-slate-300 hover:text-accent border border-transparent hover:border-slate-100 transition-all"><Save size={12} /></button>
                          </div>
                        </td>
                        <td className="px-4 py-4 text-right">
                          <div className="flex items-center justify-end gap-2">
                            <button onClick={() => void handleStatusToggle(u)} disabled={busyUserId === u.id} className={`p-1.5 rounded transition-all ${u.is_active ? 'text-emerald-500 hover:bg-emerald-50 dark:hover:bg-emerald-950/30' : 'text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-900'}`} title="Toggle Status"><Power size={14} /></button>
                            <button onClick={() => void handlePasswordReset(u)} disabled={busyUserId === u.id} className="p-1.5 rounded hover:bg-amber-50 dark:hover:bg-amber-950/30 text-amber-500 transition-all" title="Reset Password"><Key size={14} /></button>
                          </div>
                        </td>
                      </tr>
                    ))
        ...
        <aside className="space-y-6">
          <section className="card-simple">
            ...
            <form onSubmit={(e) => void handleBulkCreate(e)} className="space-y-5">
              <div>
                <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">Default Password</label>
              ...
              <div>
                <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">CSV Data (ID,SID,Name,Email,Phone)</label>
                <textarea value={bulkText} onChange={(e) => setBulkText(e.target.value)} rows={12} className="w-full text-xs rounded border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 px-3 py-2 font-mono outline-none" placeholder="student01,20250001,John Doe,j@ex.com,010-0000-0000" />
              </div>
              <button type="submit" disabled={isBulkSaving} className="btn-flat w-full h-11 text-xs font-bold uppercase tracking-widest disabled:opacity-50">
                {isBulkSaving ? 'Processing...' : 'Onboard Users'}
              </button>
            </form>
          </section>
        </aside>
