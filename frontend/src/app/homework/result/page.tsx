    ...
    <div className="max-w-4xl mx-auto py-8 px-4 space-y-10">
      <header className="flex items-center justify-between">
        <div>
          ...
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Submission Result</h1>
          <p className="text-slate-500 text-sm font-medium mt-1">
            Assignment #{submission.homework_num} · {submission.homework_title}
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 text-center">
        {[
          { label: 'Total Score', value: submission.total_score, highlight: 'text-accent' },
          { label: 'Functional', value: submission.submission_score, highlight: 'text-emerald-600' },
          { label: 'Quality', value: submission.quality_score, highlight: 'text-amber-600' },
          { label: 'Attempt', value: `#${submission.attempt_no}`, highlight: 'text-slate-900 dark:text-slate-100' },
          { label: 'Language', value: submission.language, highlight: 'text-slate-900 dark:text-slate-100' },
          { label: 'Status', value: submission.status, highlight: 'text-slate-900 dark:text-slate-100' },
        ].map((stat) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
          <section className="card-simple">
            ...
            <div className="bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded p-4 font-mono text-[13px] leading-relaxed text-slate-700 dark:text-slate-300 overflow-x-auto whitespace-pre-line min-h-[120px]">
              {submission.grader_summary || 'Grading in progress... Please wait.'}
            </div>
          </section>
          ...
          {feedback && (
            <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="card-simple bg-amber-50/30 dark:bg-amber-950/10 border-amber-100/50">
                <h2 className="text-xs font-bold uppercase tracking-widest text-amber-600 mb-4">Next Action</h2>
              ...
              {feedback.coding_rule_guides.length > 0 && (
                <div className="card-simple bg-blue-50/30 dark:bg-blue-950/10 border-blue-100/50">
                  <h2 className="text-xs font-bold uppercase tracking-widest text-blue-600 mb-4">Quality Guide</h2>
              ...
              {history.map((item) => (
                <Link key={item.id} href={`/homework/result?id=${item.id}`} className={`block p-3 rounded border transition-all ${item.id === submission.id ? 'border-accent bg-accent/5' : 'border-transparent hover:border-slate-100 dark:hover:border-slate-800 hover:bg-slate-50'}`}>
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-xs font-bold">Attempt #{item.attempt_no}</span>
                    <span className="text-xs font-mono font-bold text-slate-500">{item.total_score}</span>
                  </div>
