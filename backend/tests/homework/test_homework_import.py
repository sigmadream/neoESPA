...
def test_import_auth_requires_admin(
    ...
    res = client.post(
        "/api/admin/homeworks/import", headers=auth_headers(token)
    )
    assert res.status_code == 403


def test_admin_can_import_homework(
    client: TestClient,
    session: Session,
    create_user,
    login_user,
    ...
    monkeypatch.setattr(
        homework_router, "_get_homework_artifact_root", lambda: artifact_root
    )
    ...
    response = client.post(
        ...
        files={
            "problem_file": (
                "problem.pdf",
                _read_fixture("problem.pdf"),
                "application/pdf",
            ),
            "input_zip": (
                "inputs.zip",
                _read_fixture("inputs.zip"),
                "application/zip",
            ),
            "output_zip": (
                "outputs.zip",
                _read_fixture("outputs.zip"),
                "application/zip",
            ),
        },
    ...
    audit = session.exec(
        select(AuditLog).where(AuditLog.target_id == str(homework_num))
    ).first()
    assert audit is not None
    assert (artifact_root / str(homework_num)).exists()


def test_import_validation_zip_errors(
    client: TestClient, create_user, login_user, auth_headers, dt_string
):
...
def test_import_homework_cleans_up_artifacts_after_http_error(
    ...
    monkeypatch.setattr(
        homework_router, "upsert_homework_artifact_metadata", fail_metadata
    )
...
def test_import_homework_cleans_up_artifacts_after_unexpected_error(
    ...
    monkeypatch.setattr(
        homework_router, "upsert_homework_artifact_metadata", explode_metadata
    )
