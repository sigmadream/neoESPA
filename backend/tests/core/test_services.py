...
def test_password_hashing():
    ...
    # 3. 잘못된 비밀번호 검증
    assert (
        AuthService.verify_password("wrong_password", hashed_password) is False
    )
...
def test_system_setting_parsing_and_normalization(monkeypatch):
    ...
    assert normalize_setting_value("lint_calc_panalty", 0.75) == (
        "0.75",
        "number",
    )
    assert normalize_setting_value("lint_set_default", "on") == (
        "true",
        "boolean",
    )
