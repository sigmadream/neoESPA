...
def test_analytics_export_only_includes_latest_consented_learners(
    session, tmp_path
):
    session.add(
        Homework(
    ...
    session.add(
        Submission(
            ...
            user_id="denied",
            language="python",
            code_text="other source",
        )
    )
    session.commit()
    service = AnalyticsExportService("x" * 32, tmp_path)
    counts = service.export_jsonl(
        session, purpose="research", policy_version="v1"
    )
    rows = [
        json.loads(line)
        for line in (tmp_path / "exports/submissions.jsonl")
        .read_text()
        .splitlines()
    ]
