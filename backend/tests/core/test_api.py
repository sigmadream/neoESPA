...
def test_user_registration_password_is_hashed(client: TestClient, session: Session):
    # 1. 회원가입 요청
    user_data = {
        "id": "testuser",
        "sid": 20240001,
...
def test_login_success(client: TestClient):
    ...
    # 로그인 시도
    login_response = client.post("/api/auth/login", json={"id": "loginuser", "ps": "correct_password"})
...
def test_login_failure(client: TestClient):
    ...
    # 잘못된 비밀번호로 로그인
    login_response = client.post("/api/auth/login", json={"id": "failuser", "ps": "wrong_password"})
...
def test_protected_route_access(client: TestClient):
    ...
    login_response = client.post("/api/auth/login", json={"id": "protecteduser", "ps": "testpassword"})
    ...
    invalid_token_response = client.get("/api/users/me", headers=wrong_token_headers)
