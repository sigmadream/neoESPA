...
def _admin() -> User:
    return User(
        id="root", sid=1, ps="hash", name="Root", phone="", email="root@test",
...
def test_admin_invitation_is_single_use_and_pins_email_and_role(session):
    ...
    invitation, token = issue_admin_invitation(
        session, email="New.Admin@Test.EXAMPLE", role_name="reviewer",
        created_by="root", ttl_minutes=15,
    )
    session.commit()
    user = accept_admin_invitation(
        session, token=token, user_id="reviewer", sid=2, name="Reviewer",
        phone="010", password="strong-password",
    )
    ...
    with pytest.raises(AdminInvitationError, match="invalid"):
        accept_admin_invitation(
            session, token=token, user_id="other", sid=3, name="Other",
...
def test_admin_invitation_rejects_super_admin_role(session):
    ...
    with pytest.raises(AdminInvitationError, match="cannot be assigned"):
        issue_admin_invitation(
            session, email="other@test", role_name="super_admin",
            created_by="root", ttl_minutes=15,
        )
