...
def mock_support_dir(tmp_path):
    ...
    support.mkdir()
    (support / "week_rules.json").write_text(
        json.dumps({"pylint": {"1": ["rule1"]}}), encoding="utf-8"
    )
    (support / "pylint_message.json").write_text(
        json.dumps({"rule1": {"kor": "규칙1"}}), encoding="utf-8"
    )
    (support / "lint_rule_manual.json").write_text(
        json.dumps({"pylint": {"rule1": {"kor": "규칙1 상세"}}}),
        encoding="utf-8",
    )
    return support
...
def test_deadline_feedback_logic(feedback_service):
    ...
    hw_open = Homework(
        ...
        intro="...",
        codeName="main",
        starttime=(datetime.now(UTC) - timedelta(days=1)).strftime(
            "%Y-%m-%dT%H:%M:%S+00:00"
        ),
        deadline=(datetime.now(UTC) + timedelta(days=2)).strftime(
            "%Y-%m-%dT%H:%M:%S+00:00"
        ),
    )
    ...
    hw_soon = Homework(
        ...
        deadline=(datetime.now(UTC) + timedelta(hours=23)).strftime(
            "%Y-%m-%dT%H:%M:%S+00:00"
        ),
    )
    ...
    hw_closed = Homework(
        ...
        intro="...",
        codeName="main",
        starttime=(datetime.now(UTC) - timedelta(days=2)).strftime(
            "%Y-%m-%dT%H:%M:%S+00:00"
        ),
...
def test_build_hints_mapping(feedback_service):
    # 컴파일 실패 시 힌트
    res_compile_fail = SubmissionResult(
        submission_id=1, compile_status="failed", status="graded"
    )
    hints = feedback_service._build_hints(
        res_compile_fail, "open", None, lint_enabled=True
    )
    assert any("컴파일 오류" in h for h in hints)

    # 타임아웃 시 힌트
    res_timeout = SubmissionResult(
        submission_id=1, run_status="timeout", status="graded"
    )
    hints = feedback_service._build_hints(
        res_timeout, "open", None, lint_enabled=True
    )
    assert any("시간 초과" in h for h in hints)

    # 낮은 코드 품질 점수 힌트
    res_low_quality = SubmissionResult(
        submission_id=1, run_status="passed", quality_score=10, status="graded"
    )
    hints = feedback_service._build_hints(
        res_low_quality, "open", None, lint_enabled=True
    )
    assert any("코드 품질 점수" in h for h in hints)

    # 린트가 꺼진 과제에서는 품질 점수 힌트를 보여주지 않는다
    hints = feedback_service._build_hints(
        res_low_quality, "open", None, lint_enabled=False
    )
...
def test_build_submission_feedback_combines_deadline_notice_and_result_hints(
    ...
    homework = Homework(
        ...
        deadline=(datetime.now(UTC) + timedelta(hours=2)).strftime(
            "%Y-%m-%d %H:%M:%S"
        ),
        isLint=False,
    )
    ...
    session.add(
        Notice(
            ...
            content="...",
            is_published=True,
            date=(datetime.now(UTC) - timedelta(hours=1)).strftime(
                "%Y-%m-%d %H:%M:%S"
            ),
        )
    )
    ...
    feedback = feedback_service.build_submission_feedback(
...
def test_latest_notice_hides_future_notice_and_allows_invalid_date_fallback(
    ...
    session.add(
        Notice(
            ...
            content="...",
            is_published=True,
            date=(datetime.now(UTC) + timedelta(days=7)).strftime(
                "%Y-%m-%d %H:%M:%S"
            ),
        )
    )
...
def test_build_guides_uses_lint_week_and_message_fallback(
    feedback_service, session: Session
):
    homework = Homework(
        num=9,
    ...
    feedback_service.week_rules = {
        "pylint": {"week-custom": ["rule1", "rule-missing"]}
    }
