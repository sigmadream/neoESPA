...
@router.post("/artifact-jobs/reconcile", response_model=JudgeJobRead, status_code=202)
def create_artifact_reconciliation_job(
    ...
    job = job_service.enqueue(
        session, job_type="artifact_reconciliation", payload={"full": True},
        idempotency_key=f"artifact-reconciliation:{now_key}", priority=300,
    )
    observability_service.record_audit(
        session, actor_user_id=current_user.id,
        action_type="create_artifact_reconciliation", target_type="judge_job",
...
def list_artifact_jobs(
    ...
    jobs = session.exec(
        select(JudgeJob).where(JudgeJob.job_type == "artifact_reconciliation")
        .order_by(JudgeJob.id.desc()).limit(100)
    ).all()
...
def list_judge_workers(
    ...
    workers = session.exec(select(JudgeWorker).order_by(JudgeWorker.worker_id)).all()
    ...
    for worker in workers:
        ...
        if (now - heartbeat_at).total_seconds() > 60 and worker.status == "online":
...
@router.post("/judge-workers/{worker_id}/enable", response_model=JudgeWorkerRead)
...
@router.post("/judge-workers/{worker_id}/disable", response_model=JudgeWorkerRead)
def disable_judge_worker(
    ...
    if worker.current_job_id is not None:
        raise HTTPException(status_code=409, detail="Drain worker before disabling it")
...
def grading_metrics(
    ...
    for job in job_rows:
        if job.started_at is not None:
            ...
            wait_values.append(max(0, (started - created).total_seconds() * 1000))
    verdict_counts: dict[str, int] = {}
    for verdict in session.exec(select(GradingRun.verdict)).all():
        verdict_counts[verdict] = verdict_counts.get(verdict, 0) + 1
    problem_error_counts: dict[str, int] = {}
    for job in job_rows:
        if job.problem_id is not None and job.status in {"failed", "dead_letter"}:
            key = str(job.problem_id)
            problem_error_counts[key] = problem_error_counts.get(key, 0) + 1
    terminal = [
        job for job in job_rows if job.status in {"succeeded", "failed", "dead_letter"}
    ]
    failed_terminal = sum(job.status in {"failed", "dead_letter"} for job in terminal)
    return GradingMetricsRead(
        ...
        workers_online=sum(worker.status in {"online", "draining"} for worker in workers),
        workers_offline=sum(worker.status == "offline" for worker in workers),
        average_queue_wait_ms=(
            round(sum(wait_values) / len(wait_values), 2) if wait_values else 0.0
        ),
...
def _rejudge_statement(scope: RejudgeScope):
    ...
    if scope.problem_id is not None:
        ...
        statement = statement.where(Submission.problem_revision_id.in_(revision_ids))
    if scope.revision_id is not None:
        statement = statement.where(Submission.problem_revision_id == scope.revision_id)
    if scope.homework_num is not None:
        statement = statement.where(Submission.homework_num == scope.homework_num)
    ...
    if scope.submitted_after is not None:
        statement = statement.where(Submission.submitted_at >= scope.submitted_after)
    if scope.submitted_before is not None:
        statement = statement.where(Submission.submitted_at <= scope.submitted_before)
    return statement
...
def list_judge_jobs(
    ...
    jobs = session.exec(statement.order_by(JudgeJob.id.desc()).limit(limit)).all()
    return [job_service.to_read(job) for job in jobs]
...
def list_grading_incidents(
    ...
    statement = select(JudgeJob).where(JudgeJob.status.in_(["failed", "dead_letter"]))
    if problem_id is not None:
        statement = statement.where(JudgeJob.problem_id == problem_id)
    if worker_id:
        statement = statement.where(JudgeJob.lease_owner == worker_id)
    jobs = session.exec(statement.order_by(JudgeJob.finished_at.desc(), JudgeJob.id.desc()).limit(limit)).all()
    return [job_service.to_read(job) for job in jobs]
...
def preview_rejudge(
    ...
    ids = session.exec(
        _rejudge_statement(scope).with_only_columns(Submission.id).order_by(Submission.id)
    ).all()
...
def create_rejudge(
    ...
    ids = session.exec(
        _rejudge_statement(payload).with_only_columns(Submission.id).order_by(Submission.id)
    ).all()
    ...
    if payload.contest_id is not None:
        ...
        contest_approval = session.get(ContestOperationApproval, payload.contest_approval_id)
        if (
            ...
            raise HTTPException(status_code=409, detail="Unused contest rejudge approval is required")
    try:
        parent = job_service.enqueue(
            session,
            job_type="rejudge_batch",
            payload={
                "scope": payload.model_dump(mode="json", exclude={"reason", "idempotency_key"}),
...
def get_rejudge_job(
    ...
    if children:
        completed = sum(child.status in {"succeeded", "failed", "cancelled", "dead_letter"} for child in children)
...
def list_problem_jobs(
    ...
    statement = select(JudgeJob).where(JudgeJob.job_type == "problem_validation")
    if status:
        statement = statement.where(JudgeJob.status == status)
    if revision_id:
...
@router.get("/problem-jobs/{job_id}/events", response_model=list[JudgeJobEventRead])
