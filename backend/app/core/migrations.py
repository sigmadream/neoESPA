...
def apply_migrations(engine) -> None:
    ...
    pending = [version for version, _upgrade in MIGRATIONS if version not in applied_versions]
    if pending:
...
def _create_production_pre_migration_snapshot(engine, applied_versions: set[str]) -> None:
    from pathlib import Path

    from .config import settings

    if settings.ENVIRONMENT != "production" or engine.url.get_backend_name() != "sqlite":
        return
    database_name = engine.url.database
    if not database_name:
        return
    with engine.connect() as connection:
        user_table_count = connection.execute(text(
    ...
    CourseBundleService().create_snapshot(
        Path(database_name), course_id=settings.COURSE_ID, term=settings.COURSE_TERM,
        schema_version=max(applied_versions) if applied_versions else "legacy-unversioned",
    )
