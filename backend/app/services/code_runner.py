...
class SubprocessCodeRunner(CodeRunner):
    ...
    def _resolve_source_name(
        self, language: str, source_name: str | None
    ) -> str:
        if source_name:
...
class NsJailCodeRunner(SubprocessCodeRunner):
    ...
    def prepare_code(
        ...
        *,
        ...
        with tempfile.TemporaryDirectory(prefix="neoespa-sandbox-") as temp_dir:
            ...
            compile_command, run_command = self._sandbox_commands(
    ...
    def _sandbox_phase(
        ...
        result, _ = self._sandbox_phase_with_limit(
            command, workspace, stdin, limits
        )
        return result
    ...
    def _sandbox_phase_with_limit(
        ...
        executed = self.sandbox.run(
            command, workspace=workspace, stdin=stdin, limits=limits
        )
        return (
            PhaseExecutionResult(
                command=command,
    ...
    def _sandbox_commands(
        ...
        return ["/usr/bin/python3", "-m", "py_compile", source], [
            "/usr/bin/python3",
            source,
        ]
