    ...
    while True:
        ...
        if size > MAX_PACKAGE_BYTES:
            raise ProblemPackageError(
                "Problem package exceeds the upload limit"
            )
        chunks.append(chunk)
...
def parse_problem_package(content: bytes) -> list[PackageCase]:
    ...
    try:
        ...
        raise ProblemPackageError(
            "Problem package is not a valid ZIP"
        ) from error
    with archive:
        ...
        for info in infos:
            ...
            if (
                ...
                raise ProblemPackageError(
                    "Problem package contains an unsafe path"
                )
            unix_mode = info.external_attr >> 16
            if stat.S_ISLNK(unix_mode):
                raise ProblemPackageError(
                    "Problem package cannot contain symbolic links"
                )
            if info.filename in names:
                raise ProblemPackageError(
                    "Problem package contains duplicate paths"
                )
        ...
        if not isinstance(raw_cases, list) or not raw_cases:
            raise ProblemPackageError(
                "manifest testcases must be a non-empty list"
            )
        ...
        for raw in raw_cases:
            try:
                ...
                raise ProblemPackageError(
                    "Testcase manifest entry is invalid"
                ) from error
            if (
                not name
                or name in case_names
                or position < 1
                or position in positions
            ):
                raise ProblemPackageError(
                    "Testcase names and positive positions must be unique"
                )
            ...
            if not input_content or not output_content:
                raise ProblemPackageError(
                    "Testcase input and output files cannot be empty"
                )
            case_names.add(name)
