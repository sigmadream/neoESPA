...
ADMIN_INVITABLE_ROLES = {
    "admin", "problem_setter", "reviewer", "judge_operator", "support", "viewer"
}
...
def issue_admin_invitation(
    session: Session, *, email: str, role_name: str, created_by: str, ttl_minutes: int
    ...
    if role_name not in ADMIN_INVITABLE_ROLES:
        raise AdminInvitationError("Role cannot be assigned through an invitation")
...
def accept_admin_invitation(
    session: Session, *, token: str, user_id: str, sid: int, name: str,
    ...
    user = User(
        id=user_id, sid=sid, name=name.strip(), phone=phone.strip(), email=invitation.email,
        ps=AuthService.get_password_hash(password), user_group=invitation.role_name,
        is_active=True,
    )
    session.add(user)
    session.flush()
