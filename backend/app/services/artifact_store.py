...
class LocalArtifactStore:
    ...
    def initialize(self) -> None:
        ...
        if self.objects_root.stat().st_dev != self.staging_root.stat().st_dev:
            raise RuntimeError(
                "Artifact staging and objects must use the same filesystem"
            )
    ...
    def put_stream(
        self, stream: BinaryIO, *, max_bytes: int = MAX_ARTIFACT_BYTES
    ) -> StoredArtifact:
        self.initialize()
        digest = hashlib.sha256()
        size = 0
        fd, staging_name = tempfile.mkstemp(
            prefix="upload-", dir=self.staging_root
        )
        ...
        try:
            ...
            return StoredArtifact(
                sha256, relative.as_posix(), size, was_created
            )
        except Exception:
