...
from .services.openapi_contract import (
    breaking_changes,
    canonical_openapi,
    load_schema,
)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="neoESPA backend administration CLI"
    )
    ...
    create_admin_parser.add_argument(
        "--id", dest="user_id", required=True, help="Login ID"
    )
    create_admin_parser.add_argument(
        "--sid", type=int, required=True, help="Student or staff ID"
    )
    create_admin_parser.add_argument(
        "--name", required=True, help="Display name"
    )
    create_admin_parser.add_argument(
        "--phone", required=True, help="Phone number"
    )
    create_admin_parser.add_argument(
        "--email", required=True, help="Email address"
    )
    create_admin_parser.add_argument(
        "--password", required=True, help="Initial password"
    )
    create_admin_parser.add_argument(
        "--inactive",
        action="store_true",
    ...
    worker_parser.add_argument(
        "--worker-id",
        default=f"validation-{socket.gethostname()}",
        help="Stable worker identity",
    )
    worker_parser.add_argument(
        "--once", action="store_true", help="Process at most one job"
    )
    ...
    check_openapi = subparsers.add_parser(
        "check-openapi",
        help="Fail when the current API breaks a saved OpenAPI contract",
    )
    check_openapi.add_argument("--baseline", default="openapi.json")
    analytics_export = subparsers.add_parser(
        "export-analytics-jsonl",
        help="Export consent-filtered pseudonymous analytics data",
    )
    ...
    backfill_artifacts = subparsers.add_parser(
        ...
        help="Copy legacy files into the content-addressed store",
    )
    backfill_artifacts.add_argument("--legacy-root", required=True)
    capacity = subparsers.add_parser(
        "record-capacity-baseline",
        help="Record host capacity and course workload assumptions",
    )
    capacity.add_argument("--expected-students", type=int, required=True)
    capacity.add_argument(
        "--max-concurrent-submissions", type=int, required=True
    )
    capacity.add_argument(
        "--expected-term-submissions", type=int, required=True
    )
    snapshot = subparsers.add_parser(
        "create-course-snapshot",
        help="Checkpoint and snapshot an idle or frozen course database",
    )
    ...
    sandbox_test = subparsers.add_parser(
        ...
        help="Run hostile fixtures and write a policy-bound attestation",
    )
...
def _run_restore_course_bundle(target: str, replace: bool) -> int:
    counts = CourseBundleService().restore_snapshot(
        Path(target), replace=replace
    )
    print(
        json.dumps({"restored": True, "row_counts": counts}, ensure_ascii=False)
    )
    return 0
...
def _run_check_openapi(baseline: str) -> int:
    ...
    print(
        json.dumps(
            {"compatible": not errors, "breaking_changes": errors},
            ensure_ascii=False,
        )
    )
...
def _run_export_analytics(purpose: str, policy_version: str) -> int:
    ...
    print(
        json.dumps({"exported": True, "row_counts": counts}, ensure_ascii=False)
    )
    return 0
